    ...
    fn handle_request(&mut self, req: Request) {
        ...
        if let Some((id, params)) = cast::<GotoDefinition>(&mut req) {
            ...
            if let Some((ast, code, _)) = self.files.get(&params.text_document.uri) {
                ...
                if cfg!(feature = "nixpkgs-fmt-rnix") {
                ...
                } else if cfg!(feature = "alejandra_engine") {
                    #[cfg(feature = "alejandra_engine")]
                    match alejandra_engine::format::in_memory(
                        params.text_document.uri.to_string(),
                        code.to_string(),
                    ) {
                        (alejandra_engine::format::Status::Changed(true), new_text) => {
                            self.reply(Response::new_ok(id, TextEdit { range, new_text }))
                        }
                        (alejandra_engine::format::Status::Changed(false), _) => {
                            self.reply(Response::new_ok(id, ()))
                        }
                        (alejandra_engine::format::Status::Error(e), _) => {
